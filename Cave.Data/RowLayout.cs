...
namespace Cave.Data
{
    ...
    public sealed class RowLayout : IEquatable<RowLayout>, IEnumerable<IFieldProperties>
    {
        ...
        public static void CheckLayout(RowLayout expected, RowLayout current, Func<IFieldProperties, IFieldProperties> fieldPropertiesConversion = null)
        {
            ...
            {
                IFieldProperties expectedField = expected[i];
                IFieldProperties currentField = current[i];
        ...
        public static RowLayout CreateAlien(Type type, bool onlyPublic)
        {
            ...
            {
                FieldInfo fieldInfo = rawInfos[i];
                try
                {
        ...
        /// <returns>A new <see cref="RowLayout"/> instance.</returns>
        public static RowLayout CreateUntyped(string name, params IFieldProperties[] fields)
        {
            return new RowLayout(name, fields, null);
        }
        ...
        public static RowLayout CreateTyped(Type type, string nameOverride = null, IStorage storage = null, params string[] excludedFields)
        {
            ...
            lock (layoutCache)
            {
                string cacheName = $"{type.FullName},{nameOverride}";
                if (!layoutCache.TryGetValue(cacheName, out RowLayout result))
                {
        ...
        /// </summary>
        /// <param name="fieldName">The fieldname to search for.</param>
        ...
        void Save(DataWriter writer, IFieldProperties field)
        {
            ...
            var typeName = field.ValueType.AssemblyQualifiedName.Substring(0, field.ValueType.AssemblyQualifiedName.IndexOf(','));
